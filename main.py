...
async def websocket_endpoint(websocket: WebSocket):
    ...
    clients.append({"username": username, "websocket": websocket})  # Guardar el nombre y WebSocket
    
    # Enviar todo el historial de mensajes al nuevo cliente
    for message in chat_history:
        await websocket.send_text(json.dumps(message))  # Enviar cada mensaje al nuevo cliente
    ...
    try:
        while True:
            ...
            for client in clients:
                await client["websocket"].send_text(json.dumps(message_data))  # Enviar el mensaje a todos
